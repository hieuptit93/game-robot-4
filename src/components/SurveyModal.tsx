...
const SurveyModal: React.FC<SurveyModalProps> = ({ isOpen, onClose, onPlayAgain, gameSessionId, currentGameId, userId, age, urlParams }) => {
    ...
    const [likeAnswer, setLikeAnswer] = useState<'happy' | 'neutral' | 'sad' | null>(null);
    const [difficultyAnswer, setDifficultyAnswer] = useState<'easy' | 'normal' | 'hard' | null>(null);
    const [comment, setComment] = useState('');
    const [wantsReplay, setWantsReplay] = useState<'yes' | 'no' | null>(null);
    ...
    const handleNextFromStep1 = async (value: 'happy' | 'neutral' | 'sad') => {
        setLikeAnswer(value);
        try {
    ...
    const handleNextFromStep2 = async (value: 'easy' | 'normal' | 'hard') => {
    ...
    const handleReplayChoice = async (value: 'yes' | 'no') => {
